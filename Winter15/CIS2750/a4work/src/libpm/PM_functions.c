...
int checkRequired( Parameter * param_ll ) {
	Parameter * cur_item = param_ll;
	while ( cur_item->next != NULL ) {
		if ( cur_item->required == 1 && cur_item->status != 2 ) { //if the param is registered and hasn't been written to
			return 0;
		}
		cur_item = cur_item->next;
	}
...
void printList( ParameterList * list ) {
	ParameterList * cur_item = list;
	while ( cur_item != NULL ) {
		printf("PRINTED: %s\n", cur_item->str_val );
		cur_item = cur_item->next;
	}
...
void printStuff() {
	//this is here as it seems that if I didnt have C do something before I made calls to the Java enviroment, none of my C functions would execute until after compile() ended, weird
	//even weirder is once i had a c function execute before all calls made to java functions once, all other calls worked fine, whereever they were and even across multiple instances of dialogc
	//really weird, JNI is a dreadful place
	//ignore this in any case
	printf("C langauge\n");
}
