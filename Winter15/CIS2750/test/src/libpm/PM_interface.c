		...
		switch( inChar ) {
			...
			case '=': //should start expecting a value
				if ( parseState == 1 ) { //we're already expecting, syntax error
					fprintf(stderr, "LINE %d:SYNTAX ERROR**** Misplaced =\n", lineNum);
					free(paramName);
					free(paramValue);
					return 0;
				} else if ( strlen(paramName) == 0 ) { //paramName hasnt been written to
					fprintf(stderr, "LINE %d:PARSE ERROR**** Unnamed parameter\n", lineNum);
					free(paramName);
					free(paramValue);
					return 0;
				}
			...
			case ';': //end of line
				if ( parseState != 1 ) { //state 1 is only state where a ; is valid
					...
					fprintf(stderr, "LINE %d:PARSE ERROR**** Empty name or value\n", lineNum);
					free(paramName);
					free(paramValue);
					return 0;
				}
