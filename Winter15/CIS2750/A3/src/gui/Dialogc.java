...
public class Dialogc implements ActionListener {
	...
	*/
	private Boolean compileJ() {
		if ( project.getFile() == null ) { //new project, aka not yet saved
			if(JFileChooser.APPROVE_OPTION == fileChooser.showSaveDialog(mainWindow)) { //choosing file to creating a new one
				...
				if (1 == config.selectedParser && 0 == compile( project.getFilePath(), config.workingDirectory, project.getProjectName() )) //internal
					...
					try {
						buildProcess = Runtime.getRuntime().exec( "./bin/yadc " + project.getFilePath() + " " + config.workingDirectory + " " + project.getProjectName() );	
			...
			if (1 == config.selectedParser && 0 == compile( project.getFilePath(), config.workingDirectory, project.getProjectName() )) //internal
				...
				try { //yadc
					System.out.println("./bin/yadc " + project.getFilePath() + " " + config.workingDirectory + " " + project.getProjectName());
					buildProcess = Runtime.getRuntime().exec( "./bin/yadc " + project.getFilePath() + " " + config.workingDirectory + " " + project.getProjectName() );	
					buildProcess.waitFor();
				} catch ( Exception e ) {
					e.printStackTrace();
		...
		try {
			buildProcess = Runtime.getRuntime().exec(config.compiler + " " + config.compilerOptions + " " + config.workingDirectory + "/" + project.getProjectName() + "/" + project.getProjectName() + ".java");
			buildProcess.waitFor();
		} catch ( Exception e ) {
			e.printStackTrace();
		}
	...
	*/
	private void run() {
		try {
			buildProcess = Runtime.getRuntime().exec(config.runtime + " " + config.runtimeOptions + " -cp " + config.workingDirectory + " " + project.getProjectName() + "." + project.getProjectName() );
		} catch ( Exception e ) {
			e.printStackTrace();
		}
	}
